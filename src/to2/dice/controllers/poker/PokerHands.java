...
//    }
    ...
    public static int compare(Dice dice1, Dice dice2) {
        ...
        if (type1.getValue() > type2.getValue())
            ...
            if (type1.getValue() < type2.getValue())
                ...
                return compareSameFigures(dice1.getDice(),
                        dice2.getDice(), type1);
            }
        }
    }

    public static int compareOnCardPriorities(int[] d1, int
            [] d2) {
        int i = d1.length - 1;
        int j = 0;
        while (j <= i) {
    ...
    public static int compareSameFigures(int[] dices1, int[]
            dices2, PokerHandType type) {
        sort(dices1);
        sort(dices2);
        ...
        switch (type) {
            ...
            case TWO_PAIRS:
                ...
                if ((dices2[4] == dices2[3]) && (dices2[2]
                        == dices2[1])) {
                    d2a = dices2[4];
                    d2b = dices2[2];
                    d2c = dices2[0];
                } else {
                    if ((dices2[4] == dices2[3]) && (dices2[1]
                            == dices2[0])) {
                        d2a = dices2[4];
                        d2b = dices2[1];
                        d2c = dices2[2];
            ...
            case FOUR:
                ...
                return compareOnCardPriorities(new int[]
                        {d1a, d1b}, new int[]{d2a, d2b});
